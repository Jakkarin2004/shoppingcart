...
if(empty($_POST['id'])){
    ...
    VALUE('{$product_name}','{$price}','{$image_name }','{$detail}')") or die('query failed');
} else{
    ...
    SET  product_name='{$product_name}',price='{$price }',profile_image='{$image_name}',detail='{$detail}' 
    WHERE id='{$_POST['id']}' ") or die('query failed')  ;
